...
const AllTimeStatsContainer = (props) => {
    ...
    // const statsList = ['Most_Runs','Most_Centuries','Most_Fifties','Most_Fours','Most_Sixes']
  useEffect(() => {
    axios.get(process.env.REACT_APP_BACKEND_URL + '/stats/all-time/' + props.selection)
        .then(response => {
            ...
            switch(props.selection){
                ...
                case 'Longest_Six':
                    console.log('check')
                    setYValues(response.data.map(player => player['Distance (in meters)']))
                    break;
                default:
                    break;
          }
...
//   console.log('atData',data,'   y:',yValues)
  const ChartData = {
    labels: xValues,
    datasets: [
      {
