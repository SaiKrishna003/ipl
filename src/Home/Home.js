...
const Home = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios
      .get(process.env.REACT_APP_BACKEND_URL + "/team/" + team1Name)
      .then((response) => {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios
      .get(process.env.REACT_APP_BACKEND_URL + "/team/" + team2Name)
      .then((response) => {
  ...
  return (
    <div className="container" id="scoreContainer">
      <div className="bg-success bg-opacity-10 p-3 shadow border border-success rounded-5">
        ...
        <h3 className="text-center text-success fw-bold border-bottom border-success pb-1">
          Score
        </h3>
        <div
          className="d-flex justify-content-center align-items-center gap-2 border-success overflow-auto"
          id="score"
        >
          <div className="w-50">
      ...
      <div className="mt-2" id="scoreCard">
        <ul className="d-flex bg-primary bg-opacity-50 fw-bold shadow mb-1 list-unstyled p-1 text-center border rounded-3">
          <li
            onClick={(event) => listOnClickHandler(team1Data, event)}
            className="w-50 border-end border-3 active"
          >
            {team1Name}
          </li>
          <li
            onClick={(event) => listOnClickHandler(team2Data, event)}
            className="w-50"
          >
            {team2Name}
          </li>
